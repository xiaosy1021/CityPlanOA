<template>
  <div class="page-style">
    <div class="search-header">
      <span>模板名：</span>
      <Input clearable class="search-input-default" v-model="templateKey" @on-enter="onSearch" />
    ...
    <Modal v-model="showDialog" :mask-closable="false" width="600" title="短信管理">
      <short-msg-tmpl-form ref="shortMsgTmplForm" :datas="datas" />
...
import ShortMsgTmplForm from "./components/ShortMsgTmpl.vue";
...
export default {
  ...
  methods: {
    getTable(templateKey) {
      this.isLoading = true;
      Server.get({
        url: services.getShortMsgTmplQuery(this.AppId, this.IP),
        params: {
    ...
    onAdd() {
      ...
      this.datas = null;
      this.$refs.shortMsgTmplForm.resetForm();
    },
    //编辑
    onEdit(currentRow, index) {
      if (!currentRow) {
        ...
        this.$refs.shortMsgTmplForm.editForm(currentRow);
        this.showDialog = true;
      }
    },
    //删除
    onDelete(currentRow, index) {
      if (!currentRow) {
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            Server.get({
              url: services.getShortMsgTmplDelete(this.AppId, this.IP) + currentRow.id
            }).then(rsp => {
              if (rsp.status == 1) {
    ...
    //保存
    onSave() {
      let form = this.$refs.shortMsgTmplForm.getForm();
      Server.post({
        url: services.getShortMsgTmplExecute(this.AppId, this.IP),
        params: {

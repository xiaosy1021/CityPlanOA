<template>
  <div class="page-style">
    <div class="search-header">
      <span>模板名：</span>
      <Input clearable class="search-input-default" v-model="searchTemplateKey" @on-enter="onSearch" />
    ...
    <Modal v-model="showDialog" :mask-closable="false" width="600" title="邮件管理">
      <email-msg-tmpl-form ref="emailMsgTmplForm" :datas="datas" />
...
import EmailMsgTmplForm from "./components/EmailMsgTmpl.vue";
...
export default {
      ...
      table: {
        columns: [
          ...
          },
          {
            ...
            maxWidth: 100,
            render: (h, params) => {
              return renderHelper.renderTagForTable(h, params.row.isSysTmpl);
            },
            align: "center"
          },
          {
  ...
  methods: {
    getTable() {
      this.isLoading = true;
      Server.get({
        url: services.getEmailMsgTmplQuery(this.AppId, this.IP),
        params: {
          templateKey: this.searchTemplateKey
        }
      }).then(rsp => {
    ...
    onAdd() {
      ...
      this.datas = null;
      this.$refs.emailMsgTmplForm.resetForm();
    },
    //编辑
    onEdit(currentRow, index) {
      if (!currentRow) {
        ...
        this.$refs.emailMsgTmplForm.editForm(currentRow);
        this.showDialog = true;
      }
    },
    //删除
      ...
      if (!currentRow) {
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            Server.get({
              url:
                services.getEmailMsgTmplDelete(this.AppId, this.IP) +
                currentRow.id
            }).then(rsp => {
              if (rsp.status == 1) {
    ...
    //保存
    onSave() {
      let form = this.$refs.emailMsgTmplForm.getForm();
      Server.post({
        url: services.getEmailMsgTmplExecute(this.AppId, this.IP),
        params: {
